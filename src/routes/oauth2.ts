...
import { PASSWORD_MIN_LENGTH, FLAG_ENABLE_INVITATION_CODE } from '../utils/constants';
import * as controllers from '../controllers/oauth2';
import EditorType from '../models/Preferences/EditorType';
...
oauth2
    .route('/login')
    .post(
        [check('email', 'toast.user.email').isEmail(), check('password', 'toast.user.password_empty').not().isEmpty()],
        controllers.logIn
    );
oauth2
    .route('/profile')
...
oauth2.route('/password/update').post(
    passport.authenticate('bearer', { session: false }),
    [
        check('password', 'toast.user.password_too_short').isLength({ min: PASSWORD_MIN_LENGTH }),
        check('confirmPassword', 'toast.user.confirm_password')
...
oauth2.route('/password/reset').post(
    [
        check('email', 'toast.user.email').isEmail(),
...
oauth2.route('/verifyaccount').get([query('email', 'toast.user.email').isEmail()], controllers.verifyAccount);
oauth2.route('/sendotp').get([query('email', 'toast.user.email').isEmail()], controllers.sendOtp);
oauth2
    .route('/verifyotp')
    .get(
        [query('email', 'toast.user.email').isEmail(), query('OTP', 'toast.user.error_OTP').notEmpty()],
